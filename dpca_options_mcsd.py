...
code to perform dpca and sort by option
...
def dpca_options_mcsd(subjs=[], group='behEphys', z_flag=0):
    ...
    epoch_len = np.cumsum(epoch_len).astype(int)[:-1]
    ...
    for s_idx, subj in enumerate(subjs):
        ...
        if s_idx == 0:
            all_psths = np.zeros((num_cells, num_options, trial_len))
            all_regions = []
        else:
            all_psths = np.vstack((all_psths,  np.zeros((num_cells, num_options, trial_len))))
        ...
        for c_idx in range(num_cells):
            ...
            for o_idx in range(num_options):
                ...
                if o_idx == 0:
                    ...
                    cell_psths = np.vstack((cell_psths, option_psths))

            all_psths[c_idx,:,:] = cell_psths - cell_psths.mean()                       # mean center
            all_regions.append(ephys_data[cell_names[c_idx]].values[0][0][1][0][1:])
    ...
    for r_idx, region in enumerate(regions):
        ...
        dpca.protect = ['t']
        region_pca = dpca.fit_transform(region_psths)
        ...
        for sg_idx in range(num_options):
            ax1.plot(
                region_pca['s'][0, sg_idx, :],
                region_pca['s'][1, sg_idx, :],
                region_pca['s'][2, sg_idx, :], 
                color=colors[sg_idx])
            for e_idx in range(num_epochs):
                ax1.plot(
                    region_pca['s'][0, sg_idx, epoch_len[e_idx]],
                    region_pca['s'][1, sg_idx, epoch_len[e_idx]],
                    region_pca['s'][2, sg_idx, epoch_len[e_idx]], 
                    'o', markerfacecolor=colors[sg_idx],markeredgecolor=colors[sg_idx], markersize=5,label='_nolegend_')
        ...
        # # Save as MP4
        # ani.save("C:\\Users\\cooper\\Desktop\\oLab\\mcsd\\presentations\\25.6.9 lab meeting\\dpca_options_sma.gif", fps=20, dpi=200, bitrate=1800)

        # ax2 = fig.add_subplot(1,2,2)
        # explained_var = dpca.explained_variance_ratio_
        # num_margs = len(explained_var)
        # colors = plt.cm.cool(np.linspace(0, 1, num_margs))
        # for m_idx, key in enumerate(explained_var):
        #     plt.plot(np.cumsum(explained_var[key]), color=colors[m_idx], label=key)

        # ax2.set_xlabel('principal component')
        # ax2.set_ylabel('explained variance ratio')
        # plt.legend(explained_var.keys())
...
if __name__ == "__main__":
    ...
    result = dpca_options_mcsd(subjs=[], group=group, z_flag=z_flag)