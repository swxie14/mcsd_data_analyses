...
def goal_context_subj(subj=[], group='behEphys', behavior_mdl='det_hmbOforgetFixedEtaBetaExplorBias', var_names=[], z_flag=0, plot_flag=0):
    if not subj:
    ...
    df['currSubgoal'] = mdl_data['currSg'].values
    df['goalAmt'] = mdl_data['goalAmt'].values
    ...
    # 3 coefficients: var_name, currSubgoal, goalAmt (excluding intercept)
    coefs = np.full((num_epochs, num_cells, len(var_names), 3), np.nan)
    p_vals = np.full((num_epochs, num_cells, len(var_names), 3), np.nan)
    is_sig = np.full((num_epochs, num_cells, len(var_names), 3), np.nan)
        ...
        for e_idx in range(num_epochs):
            ...
            if np.sum(epoch_spikes == 0) / len(epoch_spikes) < 0.5:
                ...
                for v_idx, var_name in enumerate(var_names):
                    formula = f"spikes ~ {var_name} + currSubgoal + goalAmt"
                    ...
                    try:
                        ...
                        is_sig[e_idx, c_idx, v_idx, :] = (mdl.pvalues.values[1:] < 0.05).astype(int)  # Skip intercept (index 0)
                    except Exception as e:
                        print(f"Error fitting model for cell {c_idx}, epoch {epoch_names[e_idx]}, variable {var_name}: {str(e)}")
    ...
    if plot_flag:
        # Calculate significant encoding percentages for main variables only
        frac_sig = np.zeros((num_vars, num_epochs))
        for e_idx in range(num_epochs):            
            for v_idx in range(num_vars):                
                p_val_tmp = p_vals[e_idx,:, v_idx, 0]  # Index 0 = main variable coefficient
        ...
        import os
        output_dir = 'mdl_analyses/neural_encoding_results'
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f'{subj}_neural_encoding_results.png')
...
if __name__ == "__main__":
    ...
    # Get all subjects automatically from the data directory
    data_dir = '/Users/samuelxie/Desktop/oLab/behEphys/'
    subjects = []
    for item in os.listdir(data_dir):
        if item.startswith('subj_') and os.path.isdir(os.path.join(data_dir, item)):
    ...
    for subj in subjects:
        ...
        try:
            result = goal_context_subj(subj=subj, group=group, behavior_mdl=behavior_mdl, 
                                               var_names=var_names, z_flag=z_flag, plot_flag=plot_flag)
